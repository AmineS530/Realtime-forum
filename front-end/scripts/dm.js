...
socket.onmessage = function (event) {
    ...
    discussion.innerHTML += ['system',uname,discussion.previousElementSibling.value].includes(msg.sender)?
                            ...
                            `<li>RECEIVED A NEW message from ${msg.sender}.</li>`;
};
...
function changeDiscussion(elem) {
    ...
    fetch('/api/v1/get/history', {
        ...
        .then(data => {
            ...
            elem.nextElementSibling.nextElementSibling.attributes.value.value = elem.value
        })
...
function  sendDm(event) {
    ...
    let message = new Message("guest1",event.target[0].value);
    message.send()
}
